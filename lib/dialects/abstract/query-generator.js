    ...
    */
    ...
    whereQuery: function(where) {
      ...
      _.forIn(where, function (value, key) {
        items.push(self.whereItemQuery(value, key));
      });

      return items.length && "WHERE " + items.filter(function (item) {
        return item;
      }).join(' AND ') || '';
    },
    whereItemQuery: function(value, key) {
      return [this.quoteIdentifier(key), this.escape(value)].join(" = ");
    },
